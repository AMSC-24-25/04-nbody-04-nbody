...
with open(input_file, 'r') as f:
    ...
    # Update the cutoff radius calculation
    cutoff_radius = int(2048 * ((cutoff or 0) / box_size))
    ...
    for frame_section in frame_sections:
        ...
        for line in frame_section:
            ...
            # Also update the coordinate calculations
            center_x = int(2048 * (float(center_x) / box_size))
            center_y = int(2048 * (float(center_y) / box_size))
            

            drawer.ellipse(circle_to_box(center_x, center_y, cutoff_radius), fill='yellow')
            drawer.ellipse(circle_to_box(center_x, center_y, 1), fill='black')
